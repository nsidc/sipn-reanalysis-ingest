...
# Create a xarray dataarray with new array, reassign attributes
   T=xr.DataArray(t3n,coords={'lev1':lev1,'y':y,'x':x},dims=['lev1','y','x'])
   T=T.assign_attrs({'long_name': 'Temperature', 'units': 'K'})
   del t1,t2,t3n

# Specific humidity
   t1=dsin.SPFH_P0_L103_GLL0
   t2=dsin.SPFH_P0_L100_GLL0
   t3n=make_new3d(t1,t2)
   SH=xr.DataArray(t3n,coords={'lev1':lev1,'y':y,'x':x},dims=['lev1','y','x'])
   SH=SH.assign_attrs({'long_name': 'Specific humidity', 'units': 'kg kg-1'})
   del t1,t2,t3n

# Relative humidity
   t1=dsin.RH_P0_L103_GLL0
   t2=dsin.RH_P0_L100_GLL0
   t3n=make_new3d(t1,t2)
   RH=xr.DataArray(t3n,coords={'lev1':lev1,'y':y,'x':x},dims=['lev1','y','x'])
...
# purposes
   u1=dsin.UGRD_P0_L103_GLL0
   u2=dsin.UGRD_P0_L100_GLL0
   u3n=make_new3d(u1,u2)
   U=xr.DataArray(u3n,coords={'lev2':lev2,'y':y,'x':x},dims=['lev2','y','x'])
   U=U.assign_attrs({'long_name': 'U-component of wind', 'units': 'm s-1'})

   v1=dsin.VGRD_P0_L103_GLL0
   v2=dsin.VGRD_P0_L100_GLL0
   v3n=make_new3d(v1,v2)
   V=xr.DataArray(v3n,coords={'lev2':lev2,'y':y,'x':x},dims=['lev2','y','x'])
...
# Calculate wind speed
   ...
   t3n[3,:,:]=wspd1[:,:]
   t3n[0:3,:,:]=wspd2[:,:]
   WSPD=xr.DataArray(t3n,coords={'lev2':lev2,'y':y,'x':x},dims=['lev2','y','x'])
...
# Rename variables for remaining variables
   t1=dsin.rename_vars({'HGT_P0_L100_GLL0':'HGT','PRMSL_P0_L101_GLL0':'MSLP','PWAT_P0_L200_GLL0':'PWAT'})
   hgt=t1.HGT
   pwat=t1.PWAT
   slp=t1.MSLP
...
# Create the dataset that will finally be written out to the netcdf file!
   dataout=xr.Dataset(
      data_vars={'T':T,'U':U,'V':V,'HGT':hgt,'SH':SH,'RH':RH,'MSLP':slp,
                 'PWAT':pwat,'WSPD':WSPD},
